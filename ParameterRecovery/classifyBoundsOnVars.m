...
if any(isnan(lb)) || any(isnan(ub))
    error(message('optimlib:classifyBoundsOnVars:NaNBounds'))
end
% Check for +Inf lb and -Inf ub
if any(lb == Inf)
    error(message('optimlib:classifyBoundsOnVars:PlusInfLb'))
end
if any(ub == -Inf)
    error(message('optimlib:classifyBoundsOnVars:MinusInfUb'))
end
% Check for fixed variables equal to Inf or -Inf
if any(lb(:) == ub(:) & isinf(lb(:)))
    error(message('optimlib:classifyBoundsOnVars:FixedVarsAtInf'))
end
