...
library(readxl)
mle_effr <- read_excel(here("data_summary/mle_corr_matrix_repeat.xlsx"))
mle_effr <- mle_effr %>% select(-...1)
rownames(mle_effr) <- c("Cal", "GDP", "Mem", "Vis")

png(here::here("figs", "MLE_heatmap_repeat.png"))
heatmap(as.matrix(mle_effr), Rowv = NA, Colv = NA, 
        col = colorRampPalette(c("blue", "white", "purple"))(25), 
        scale="row", margins = c(5,10))
dev.off()
```