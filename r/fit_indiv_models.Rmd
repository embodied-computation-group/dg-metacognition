...
pacman::p_load(afex, readr, dplyr, tidyr, ggplot2, cowplot, devtools, here, tidyverse, magrittr, reshape2, rjags, coda, lattice, ggmcmc, foreach, doParallel, ggpubr, ggpol, patchwork, ggcorrplot, ggstatsplot, GGally, BayesFactor, gridExtra, viridis)
...
ggSplit <- ggplot(data=ConfByError, 
       aes(x = modality, y = avgCF_Corr, fill=accuracy)) + 
  geom_boxjitter(jitter.shape = 21, notch = TRUE, width = .25, errorbar.draw = TRUE, errorbar.length = .2) +
    theme_cowplot() +
...
p8<-ggplot(metadata_exc, aes(x = modality, y = mratio, fill= modality)) +
  geom_boxjitter(outlier.shape = NA,
                 jitter.shape = 21, notch = TRUE, width = .25, errorbar.draw = TRUE, errorbar.length = .2) +
  guides(color="none") +
...
ggsave(here("figs", "Figure 2.png"))#, height = 7, width = 12) 
```
...
ggsave(here("figs", "Figure 4.png"), height = 8, width = 8)  
