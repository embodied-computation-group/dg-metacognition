...
nmetadata <- metadata%>% mutate(modality =recode_factor(modality, memory = "1", vision = "2", Calories = "3", GDP = "4", default = NULL, .missing = NULL, .ordered = FALSE)) #Rename factor levels. Is this necessary?  
...
for 
subdata <- trials2counts(nmetadata$signal, metadata$response, metadata$confidence, 7, padAmount = 1,)

nR_S1 <- unlist(subdata[1], recursive = TRUE, use.names = TRUE)
nR_S2 <- unlist(subdata[2], recursive = TRUE, use.names = TRUE)
