...
pacman::p_load(afex, readr, dplyr, tidyr, ggplot2, cowplot, devtools, here, tidyverse, magrittr, reshape2, rjags, coda, lattice, broom, ggmcmc, foreach, doParallel, ggpubr, ggpol, patchwork, ggcorrplot, ggstatsplot, GGally)
...
p5<-ggplot(metadata_exc, aes(x = modality, y =c)) +
  geom_boxjitter() +
  ggtitle("Criterion Filtered")

p6<-ggplot(metadata_exc, aes(x = modality, y = avg_conf)) +
  geom_boxjitter() +
  ggtitle("Metacognitive Bias Filtered")

p7<-ggplot(metadata_exc, aes(x = modality, y = mda)) +
  geom_boxjitter() +
  ggtitle("Metacognitive Sensitivity Filtered")

p8<-ggplot(metadata_exc, aes(x = modality, y = da)) +
  geom_boxjitter() +
  #ylim(-2,2)+
  ggtitle("Cognitive Sensitivity Filtered")
...
subjects = unique(metadata_exc$subject)
modalities = as.character(unique(metadata_exc$modality))
