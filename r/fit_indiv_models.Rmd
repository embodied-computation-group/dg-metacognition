...
pacman::p_load(afex, readr, dplyr, tidyr, ggplot2, cowplot, devtools, here, tidyverse, magrittr, reshape2, rjags, coda, lattice, ggmcmc, foreach, doParallel, ggpubr, ggpol, patchwork, ggcorrplot, ggstatsplot, GGally, BayesFactor, gridExtra, viridis, gghalves)
...
CHANGE THE NAMES OF THE LEGEND SOMEHOW 
```{r}
...
GroupStair <- ggplot(metadataPlotMean, aes(x = trial_bin, y = mean, group = modality, color= modality)) +
  geom_line() +
  geom_point() + 
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), size = 1, width=0) +
...
tmp_data <- metadata  %>%
  ...
  mutate(accuracy = recode_factor(accuracy, '1' = "hit", '0' = "miss"))
...
ggSplit <- ggplot(data=ConfByError, 
       aes(x = modality, y = avgCF_Corr, fill=accuracy), alpha = 0.7) + 
#  geom_boxjitter(jitter.shape = 21, notch = TRUE, width = .5, errorbar.draw = TRUE, errorbar.length = .2,
...
#                 jitter.shape = 21, notch = TRUE, width = .25, errorbar.draw = TRUE, errorbar.length = .2, alpha = #.7) +
 geom_half_violin(alpha = 0.7)+
  geom_half_point(shape = 21, alpha = 0.7)+
  geom_half_boxplot(outlier.shape = NA, 
                    side = "l", notch = TRUE, width = 0.25,
...
p9<-ggplot(metadata_exc, aes(x = modality, y = da, fill= modality)) +
 # geom_boxjitter(outlier.shape = NA,
#                 jitter.shape = 21, notch = TRUE, errorbar.draw = TRUE, errorbar.length = .2, alpha = .7) +
   geom_half_violin(alpha = 0.7)+
  geom_half_point(shape = 21, alpha = 0.7)+
  geom_half_boxplot(outlier.shape = NA, 
                    side = "l", notch = TRUE, width = 0.25,
...
ggsave(here("figs", "Figure 2.png"), height = 7, width = 4) 
```
...
Rho1 <- mcmc.sample %>%
  ...
  apatheme +
  xlim(c(-0.25, 1)) +
  labs(title = "Memory and Calories",
       subtitle= paste(H1title))+
...
Rho2 <- mcmc.sample %>%
  ...
  apatheme +
    labs(title = "Memory and GDP",
       subtitle= paste(H2title))+
  theme(plot.title = element_text( size = 12),
  plot.subtitle = element_text(size = 8)) +
  xlim(c(-0.25, 1)) +
 # ylim(c(0, 2500)) +
...
Rho3 <- mcmc.sample %>%
  ...
  apatheme +
  xlim(c(-0.25, 1)) +
      labs(title = "Memory and Vision",
       subtitle= paste(H3title))+
  theme(plot.title = element_text( size = 12),
  plot.subtitle = element_text(size = 8)) +
  #ylim(c(0, 2500)) +
  ylab("Sample count") +
...
Rho4 <- mcmc.sample %>%
  ...
  apatheme +
        labs(title = "GDP and Calories",
       subtitle= paste(H4title))+
  theme(plot.title = element_text( size = 12),
  plot.subtitle = element_text(size = 8)) +
  xlim(c(-0.25, 1)) +
 # ylim(c(0, 2500)) +
...
Rho5 <- mcmc.sample %>%
  ...
  apatheme +
     labs(title = "Calories and Vision",
       subtitle= paste(H5title))+
  theme(plot.title = element_text( size = 12),
  plot.subtitle = element_text(size = 8)) +
  xlim(c(-0.25, 1)) +
 # ylim(c(0, 2500)) +
...
Rho6 <- mcmc.sample %>%
  ...
  apatheme +
     labs(title = "GDP and Vision",
       subtitle= paste(H6title))+
  theme(plot.title = element_text( size = 12),
  plot.subtitle = element_text(size = 8)) +
  xlim(c(-0.25, 1)) +
 # ylim(c(0, 2500)) +
...
ggsave(here("figs", "Figure 4.png"), height = 7, width = 7)  
