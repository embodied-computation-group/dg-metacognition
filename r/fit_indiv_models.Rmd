...
summarise(mean.acc = mean(accuracy), sd.acc = sd(accuracy), n.acc = 316) %>% # hardcoded, fix soon
...
f2
ggsave(here("figs", "Figure 2.png"), height = 16.8, width = 16.8, units = "cm") 
```
...
p3 <- ggplot(data=ConfByError, 
  ...
  geom_half_violin(side = "l", fill = NA)+
  geom_half_point(aes(color = modality), range_scale = 1/2, alpha = 1/4,
                  side = "l", position = position_dodge(c(3/4,3/4)))+
  geom_half_boxplot(outlier.shape = NA, 
                    side = "r", notch = TRUE, width = 1/2,
                    alpha = 3/4, color = "black", show.legend = FALSE, 
...
p4<-ggplot(metadata_exc, aes(x = modality, y = mratio, fill= modality)) +
   ...
   #geom_half_violin(alpha = 0.7, side = "r")+
   geom_half_point(range_scale = 8/10, shape = 21, alpha = 1/10, side = "l")+
   geom_half_boxplot(outlier.shape = NA, notch = TRUE, width = 1/4, alpha = 3/4,
                     color = "black", side = "r")+
   guides(color="none") +
...
ggsave(here("figs", "Figure 3.png"), height = 7, width = 7)  
...
ggsave(here("figs", "Figure 3A.png"), height = 16.8, width = 16.8, units = "cm", dpi = 300)  
