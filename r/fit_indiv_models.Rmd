...
s11E <-scatter(scat2E, scat2E$mratio_memory, scat2E$mratio_Calories) + theme(plot.title = element_text( size = 13)) +
...
s16E <- scatter(scat2E, scat2E$mratio_Calories, scat2E$mratio_vision) + theme(plot.title = element_text( size = 13)) +
...
Averages of performance for descriptive statistics: 
```{r}
...
Cog12[1,3] <- "d' (Bayes)"
Cog12[1,4] <- "d' (Bayes)"
Cog12[1,5] <- "Criterion (Bayes)"
Cog12[1,6] <- "Criterion (Bayes)"
Cog12[1,7] <- "n"
Cog12[1,8] <- "d' (MLE)"
Cog12[1,9] <- "d' (MLE)"
...
All MLE Correlations: 
```{r}

#combine upper and lower limit of CI into one. 
MLEcorr <- corrtable %>% mutate(MC_CI = paste(round(corrtable$CI_lowerMC,3), round(corrtable$CI_upperMC,3), sep=";")) %>%
...
CorT[2,5] <- table2[5,2] # Cal and GDP
CorT[3,5] <- table2[1,2] # Cal and Mem
CorT[4,5] <- table2[6,2] # Cal and Vis
CorT[3,6] <- table2[3,2] # GDP and Mem
CorT[4,6] <- table2[4,2] # GDP and Vis
CorT[4,7] <- table2[2,2] # Mem and Vis
...
CorT <- CorT %>% mutate(Modality =X1, n=X2, M=X3, SD=X4, Calories = X5, GDP = X6, Memory =X7) %>%
  select(Modality, n, M, SD, Calories, GDP, Memory)
...
MLEMT [2,5] <- paste0(round(table3[5,2],3)) # Cal and GDP
MLEMT [3,5] <- paste0(round(table3[1,2],3), "*") # Cal and Mem
MLEMT [4,5] <- paste0(round(table3[6,2],3)) # Cal and Vis
MLEMT [3,6] <- paste0(round(table3[3,2],3), "*") # GDP and Mem
MLEMT [4,6] <- paste0(round(table3[4,2],3)) # GDP and Vis
MLEMT [4,7] <- paste0(round(table3[2,2],3)) # Mem and Vis

#  * : under 0.05 - can be seen in dataframe table3. 

#Rename and select Columns: 
MLEMT  <- MLEMT %>% mutate(Modality =X1, n=X2, M=X3, SD=X4, Calories = X5, GDP = X6, Memory =X7) %>%
  select(Modality, n, M, SD, Calories, GDP, Memory)
...
MLEBI [2,5] <- paste0(round(table3a[5,2],3), "***") # Cal and GDP
MLEBI [3,5] <- paste0(round(table3a[1,2],3), "***") # Cal and Mem
MLEBI [4,5] <- paste0(round(table3a[6,2],3), "***") # Cal and Vis
MLEBI [3,6] <- paste0(round(table3a[3,2],3), "***") # GDP and Mem
MLEBI [4,6] <- paste0(round(table3a[4,2],3), "***") # GDP and Vis
MLEBI [4,7] <- paste0(round(table3a[2,2],3), "***") # Mem and Vis

#  * : under 0.05 - can be seen in dataframe table3. 

#Rename and select Columns: 
MLEBI  <- MLEBI %>% mutate(Modality =X1, n=X2, M=X3, SD=X4, Calories = X5, GDP = X6, Memory =X7) %>%
  select(Modality, n, M, SD, Calories, GDP, Memory)
...
MLESE [2,5] <- paste0(round(table3b[5,2],3)) # Cal and GDP
MLESE [3,5] <- paste0(round(table3b[1,2],3), "**") # Cal and Mem
MLESE [4,5] <- paste0(round(table3b[6,2],3), "*") # Cal and Vis
MLESE [3,6] <- paste0(round(table3b[3,2],3), "*") # GDP and Mem
MLESE [4,6] <- paste0(round(table3b[4,2],3)) # GDP and Vis
MLESE [4,7] <- paste0(round(table3b[2,2],3), "*") # Mem and Vis

#  * : under 0.05 - can be seen in dataframe table3. 

#Rename and select Columns: 
MLESE  <- MLESE %>% mutate(Modality =X1, n=X2, M=X3, SD=X4, Calories = X5, GDP = X6, Memory =X7) %>%
  select(Modality, n, M, SD, Calories, GDP, Memory)
...
table4 <- RhoHDI %>% mutate(Pair= c("Memory & Calories", "Memory & Vision", "Memory & GDP", 
...
HMT [2,5] <- paste0(round(table4[5,6],3), " ", table4[5,8]) # Cal and GDP
HMT [3,5] <- paste0(round(table4[1,6],3), " ",table4[1,8]) # Cal and Mem
HMT [4,5] <- paste0(round(table4[6,6],3), " ",table4[6,8]) # Cal and Vis
HMT [3,6] <- paste0(round(table4[3,6],3), " ",table4[3,8]) # GDP and Mem
HMT [4,6] <- paste0(round(table4[4,6],3), " ",table4[4,8]) # GDP and Vis
HMT [4,7] <- paste0(round(table4[2,6],3), " ",table4[2,8]) # Mem and Vis
...
#Change labels
BayesSen <- BayesSen %>% mutate(`meta-d'` = `Bayes Factor`) %>% select(-`Bayes Factor`)
BayesEff <- BayesEff %>% mutate(`meta-d/d''` = `Bayes Factor`) %>% select(-`Bayes Factor`)
BayesBia <- BayesBia %>% mutate(`Avg. Conf` = `Bayes Factor`) %>% select(-`Bayes Factor`)

#Join the dataframes:
Bayes1 <- left_join(BayesSen, BayesEff, 
                     by=c("Pair"="Pair"))
Bayes <- left_join(Bayes1 , BayesBia, 
                     by=c("Pair"="Pair"))
#write csv
write.csv(Bayes ,file= here("tables", 'MLECorrBayes.csv'))
