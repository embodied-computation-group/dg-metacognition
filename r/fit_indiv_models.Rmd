...
group_by(subject) #create a trial number column
  ...
  metadataPlot <- metadataPlot %>%group_by(modality) %>% mutate(trial_bin = ntile(trial_number, n = 4)) #split in the four bins according to task 
  #(note different number of trials)
...
#plot:   
GroupStair <- ggplot(metadataPlotMean, aes(x = trial_bin, y = mean, group = modality, color= modality)) +
  geom_line() +
  geom_point() + 
...
ggSplit<-ggplot() + geom_boxjitter(data=AvgConfCorr, aes(x = modality, y = avgCF_Corr, color="Correct")) +
  geom_boxjitter(data=AvgConfInCo, aes(x = modality, y = avgCF_Incor, color="Incorrect"))+
    theme_bw() +
   labs(x = "Average Confidence",
         y = "Modality", 
        color = "Legend") + 
    ggtitle("Confidence across Trial Types")
ggSplit
...
p8<-ggplot(metadata_exc, aes(x = modality, y = mratio, color= modality)) +
  geom_boxjitter() + theme_bw() + guides(color="none") +
  ggtitle("Metacognitive Efficiency")+
  ylab("Meta-d'/d'") + xlab("Modality")
p9<-ggplot(metadata_exc, aes(x = modality, y = da, color= modality)) +
  geom_boxjitter() + theme_bw() + guides(color="none") +
...
s11 <-scatter(scat2, scat2$mratio_memory, scat2$mratio_Calories) +
  scale_y_continuous("Calories")+scale_x_continuous("Memory") + ggtitle(paste(S1title))
s12 <- scatter(scat2, scat2$mratio_memory, scat2$mratio_vision) +
  scale_y_continuous("Vision")+scale_x_continuous("Memory") + ggtitle(paste(S2title))
s13 <- scatter(scat2, scat2$mratio_memory, scat2$mratio_GDP) +
  scale_y_continuous("GDP")+scale_x_continuous("Memory") + ggtitle(paste(S3title))
s14 <- scatter(scat2, scat2$mratio_GDP, scat2$mratio_vision) + 
  scale_y_continuous("Vision")+scale_x_continuous("GDP") + ggtitle(paste(S4title))
s15 <- scatter(scat2, scat2$mratio_GDP, scat2$mratio_Calories) + 
  scale_y_continuous("Calories")+ scale_x_continuous("GDP") + ggtitle(paste(S5title))
s16 <- scatter(scat2, scat2$mratio_Calories, scat2$mratio_vision) + 
  scale_y_continuous("Vision")+scale_x_continuous("Calories") + ggtitle(paste(S6title))
...
```{r}
Figure2 <- ggarrange(p9, GroupStair)  
  annotate_figure(Figure2, top = text_grob("Figure 2", color = "black",face = "bold", size = 14))
...
message("OBS: You need to fit the hierarhical model using the script called 'fit_hierarhicical_metamodel.rmd' - this will take a LONG time")
...
#individual rho plots: 

#Plot titles: 
meanRho <- mcmc.rho %>% group_by(Parameter) %>% summarise(meanRho= mean(value, na.rm=TRUE)) 


S1Rtitle <- mcmc.rho %>% filter(Parameter=="rho[1]") %>% summarise(meanRho= mean(value, na.rm=TRUE)) 
  
  
  meanRho %>%  %>% paste("r =", round(meanRho, digits=3))
...
RhoPlot <- ggarrange(Rho1, Rho3, Rho2, Rho6, Rho4, Rho5)
...
ggsave(here("figs", "mrho+mlecorr.png"), height = 7, width = 12)
...
Figure 4
```{r}
Figure4 <- ggarrange(p8, ggSplit)
  annotate_figure(Figure4, top = text_grob("Figure 4", color = "black",face = "bold", size = 14))
...
MLE individual fit Correlations and figure 3 without Mratio outliers: 
```{r}
#exclude those where mratio could not be estimated  
...
CorExcMLE<-annotate_figure(comPlotE, top = text_grob("Metacognitive Efficiency without outliers", color = "black", face = "bold", size = 14))   

#figure 3 with this instead: 
...
ggsave(here("figs", "mrho+mlecorrExcOutliers.png"), height = 7, width = 12)
