...
ggplot(data=ConfByError, 
       aes(x = modality, y = avgCF_Corr, fill=accuracy)) + 
  geom_boxjitter(notch = TRUE, jitter.shape = 1, outlier.shape = NA) +
    theme_cowplot() +
...
p9<-ggplot(metadata_exc, aes(x = modality, y = da, fill= modality)) +
  geom_boxjitter(outlier.shape = NA, jitter.shape = 1, notch = TRUE) +
  guides(fill="none") +
...
p9 + GroupStair + plot_annotation(
  title = 'Figure 1: Hit Rate Control',
  tag_levels = 'A',
...
ggsave(here("figs", "Figure 4.png"), height = 7, width = 12)  
