...
metacog_group_data <- read_csv(here("data_summary", "/MRatio_GroupEst.csv"))
...
names(metacog_group_data)[names(metacog_group_data) == "pp"] <- "Sid"

all_data <- full_join(survey_data_pre, survey_data_post, by = "Sid")
all_data <- full_join(all_data, metacog_data, by = "Sid")
all_data <- full_join(all_data, metacog_group_data, by = "Sid")
...
out_data <- all_data %>%
  ...
  rowwise() %>% 
  mutate(self_vis_avg=mean(c(self_visual_post,self_visual_pre))) %>% 
  mutate(self_mem_avg=mean(c(self_memory_post,self_memory_pre))) %>% 
  mutate(self_calorie_avg=mean(c(self_calories_post,self_calories_pre))) %>% 
  mutate(self_gdp_avg=mean(c(self_gdp_post,self_gdp_pre)))
...
# export data
#write_csv(all_data, here("data_summary_v1", "all_group_data.csv"))
...
set.seed(123)
parallel = fa.parallel(probe_data)

mw_model = principal(probe_data, nfactors = 5)
...
save_factor_data <- full_join(out_data, scores, by = "Sid")
save_factor_data <- filter(save_factor_data, gender != "non-binary")
write_csv(save_factor_data, here("data_summary", "all_group_datav1.csv"))
