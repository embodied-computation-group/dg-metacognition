...
#Sort number of decimals: 
tableSH <- tableSH %>% mutate(Memory = round(Memory,3),
...
tableSH <- tableSH %>% mutate(P_valueM = as.character("<.001"),
                              P_valueC = as.character("<.001"),
                              P_valueG = as.character("<.001"),
                              P_valueV = as.character("<.001"))
#Make a dataframe with right format
tableSHT <- data.frame(Domain=character(),
                 r=double(),
                 p=double(),
                stringsAsFactors=FALSE)

tableSHT <- tableSHT %>% rename('p (CI)'= p)

#Put values into dataframe
tableSHT[1,] <- list("Memory", tableSH$Memory, paste0(tableSH$P_valueM, "(", (tableSH$Mem_CI), ")"))
tableSHT[2,] <- list("Calories", tableSH$Calories, paste0(tableSH$P_valueC, "(", (tableSH$Cal_CI), ")"))
tableSHT[3,] <- list("GDP", tableSH$GDP, paste0(tableSH$P_valueG, "(", (tableSH$GDP_CI), ")"))
tableSHT[4,] <- list("Vision",tableSH$Vision, paste0(tableSH$P_valueV, "(", (tableSH$Vis_CI), ")"))


#Add APA Format:
SpliHalf<-apa(tableSHT, "Table ??: Split Half Trials analyses of Metacognitive Efficiency")
