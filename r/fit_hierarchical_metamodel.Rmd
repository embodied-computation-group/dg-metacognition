...
Rho_plot <- mcmc.sample %>%
  filter(Parameter == corr_str) %>%
  ggplot(aes(value)) +
  geom_histogram(binwidth = 0.03, fill = "blue", colour = "grey", alpha = 0.5) +
  geom_vline(xintercept = stat$mean[stat$name == corr_str],linetype="dashed", size = 1.5) +
  geom_segment(aes(x = HDI$lower[HDI$name == corr_str], y = 50, xend = HDI$upper[HDI$name == corr_str], yend = 50), colour = "white", size = 2.5) +
  xlim(c(-1, 1)) +
  ylab("Sample count") +
