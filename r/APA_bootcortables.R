...
#' }
#' @export
apa.bootcor.table<-function(data, filename = NA, table.number = NA, show.conf.interval = TRUE, show.sig.stars = TRUE, landscape = TRUE) {
  ...
  for (i in 1:number_variables) {
    ...
    for(j in 1:number_variables) {
      if ((j<i)) {
        x <- data[,i]
        y <- data[,j]
        bootdata <- as.matrix(data)
        ctest      <- corci(bootdata[,i], bootdata[,j], method = "spearman", nboot = 10000)
