...
H1title <- paste("Rho =",round(meanRhoM$value, digits=3))
H2title <- paste("Rho =",round(meanRhoC$value, digits=3))
H3title <- paste("Rho =",round(meanRhoG$value, digits=3))
H4title <- paste("Rho =",round(meanRhoV$value, digits=3))

R1 <- RhoPlot(mcmc.sampleM, "rho") + labs(title = "Memory", subtitle= paste(H1title))
R2 <- RhoPlot(mcmc.sampleC, "rho") + labs(title = "Calories", subtitle= paste(H2title))
R3 <- RhoPlot(mcmc.sampleG , "rho") + labs(title = "GDP", subtitle= paste(H3title))
R4 <- RhoPlot(mcmc.sampleV , "rho") + labs(title = "Vision", subtitle= paste(H4title))

#Bayesian Hierarhical fit: 
RhoPlot <- ggarrange(R1, R3, R2, R6, R4)
